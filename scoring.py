...
def score_market(city: str, weather: dict[str, Any], market: dict[str, Any]) -> dict[str, Any]:
    ...
    if weather.get("current_temp_f") is None:
        warnings.append("Missing current NWS observation.")
    if weather.get("forecast_high_f") is None:
        warnings.append("Missing forecast high.")

    missing_market = market.get("implied_probability") is None
    label = final_label(edge, confidence, volatility, rebound, liquidity, crowding, warnings, missing_market)
    ...
    return {
        ...
        "forecast_high_time": weather.get("forecast_high_time"),
        ...
        "cli_high_f": weather.get("cli_high_f"),
        ...
        "active_heating_window": weather.get("active_heating_window"),
        ...
        "raw_high_so_far_f": weather.get("raw_high_so_far_f"),
        "high_so_far_f": weather.get("high_so_far_f"),
        "recent_observation_points": weather.get("recent_observation_points"),
        "heating_rate_f_per_hour": weather.get("heating_rate_f_per_hour"),
...
def _build_side_signal(
    ...
    label = signal_label(edge, confidence, volatility, rebound, liquidity, crowding, side_price)
    ...
    if _live_signal_depends_on_forecast(weather, market, side):
        ...
        signal_warnings.append(
            "Live same-day market is not locked by official station high-so-far; this signal depends on forecast and should not be treated as a bet."
        )
    if side == "NO" and bucket == "LOCKED_BY_OBSERVED_HIGH":
        if label in {"WATCH", "LEAN"}:
            label = "SKIP"
        blocked_from_near_misses = True
        signal_warnings.append(
            "Official-station rounded high has reached this bucket; NO is blocked until final CLI proves otherwise."
        )
    if bucket in {"CROSSED_BUCKET", "IMPOSSIBLE_BY_OBSERVED_HIGH"}:
        blocked_from_near_misses = True
        signal_warnings.append(
            "Official-station rounded high has already moved beyond this bucket."
        )
    if source_state == "FORECAST_ONLY":
        blocked_from_near_misses = True
        signal_warnings.append("Forecast-only signal; do not bet from this without station observations or final CLI.")
    if weather.get("active_heating_window") and source_state not in {"FINAL_CLI_AVAILABLE", "FUTURE_FORECAST"}:
        blocked_from_near_misses = True
        signal_warnings.append("Heating window is still active or observation data is stale; keep live signals conservative.")
    reasons = [
    ...
    if side == "NO":
        reasons.append("NO signal means the bucket appears unlikely relative to the visible No price.")
    return {
        "city": city,
        "side": side,
        "signal_label": label,
...
def _live_signal_depends_on_forecast(weather: dict[str, Any], market: dict[str, Any], side: str) -> bool:
    ...
    if weather.get("settlement_source_status") == "cli_available":
        return False
    if weather.get("active_heating_window"):
        return True
    if side != "NO":
        return True
    high_so_far = weather.get("high_so_far_f")
    range_high = market.get("range_high_f")
    if high_so_far is None or range_high is None:
        return True
    return high_so_far <= range_high
...
def signal_label(
    ...
    if side_price is None:
        return "SKIP"
    if confidence < 60 or volatility >= 80 or rebound >= 75:
        return "AVOID"
    if liquidity >= 85:
        return "AVOID"
...
def estimate_weather_probability(weather: dict[str, Any], market: dict[str, Any] | None = None) -> int:
    ...
    forecast = weather.get("forecast_high_f")
    high_so_far = weather.get("high_so_far_f")
    ...
    reference_high = max(v for v in [forecast, high_so_far, current] if v is not None)
    margin = threshold - reference_high
    probability_below = 50 + margin * 12
    probability_below -= max(0, rebound_risk(weather) - 50) * 0.25
    probability_below -= max(0, volatility_risk(weather) - 50) * 0.15
    probability_below = _clamp(probability_below)

    if side == "above":
        return 100 - probability_below
...
def estimate_range_probability(weather: dict[str, Any], market: dict[str, Any]) -> int:
    ...
    forecast = weather.get("cli_high_f") if cli_available else None
    if forecast is None:
        forecast = weather.get("forecast_high_f")
    high_so_far = weather.get("high_so_far_f")
    current = None if weather.get("market_day_state") == "future" else weather.get("current_temp_f")
    ...
    expected = max([value for value in [forecast, high_so_far, current] if value is not None], default=forecast)
    low = market.get("range_low_f")
    high = market.get("range_high_f")

    if cli_available:
        if high is not None and forecast > high:
            return 0
        if high is None and low is not None:
            return 100 if forecast >= low else 0
        if low is None:
            return 100 if high is not None and forecast <= high else 0
        return 100 if low <= forecast <= high else 0

    if high_so_far is not None:
        if high is not None and high_so_far > high:
            return 0
        if high is None and low is not None and high_so_far >= low:
            return 100
        if high is not None and (low is None or high_so_far >= low):
            return _clamp(100 - _exceed_upper_risk(weather, high))

    stdev = 0.8 if cli_available else (weather.get("station_temp_stdev_f") or 2.0)
    disagreement = weather.get("model_disagreement_f") or 0
    sigma = max(0.8, min(6.0, stdev + (0 if cli_available else disagreement * 0.6)))
    ...
    if cli_available or weather.get("market_day_state") == "future":
        ...
        risk_penalty = (rebound_risk(weather) * 0.0015) + (volatility_risk(weather) * 0.001)
...
def source_safety_state(weather: dict[str, Any]) -> str:
    ...
    if status == "cli_available" and weather.get("cli_high_f") is not None:
        return "FINAL_CLI_AVAILABLE"
    if weather.get("high_so_far_f") is not None or weather.get("current_temp_f") is not None:
...
def bucket_state(weather: dict[str, Any], market: dict[str, Any]) -> str:
    ...
    if weather.get("settlement_source_status") == "cli_available" and weather.get("cli_high_f") is not None:
        return "FINAL_YES" if _value_in_bucket(weather["cli_high_f"], low, high) else "FINAL_NO"

    high_so_far = weather.get("high_so_far_f")
    if high_so_far is None:
        return "UNRESOLVED_BUCKET"
    if high is not None and high_so_far > high:
        return "IMPOSSIBLE_BY_OBSERVED_HIGH" if low is None else "CROSSED_BUCKET"
    if high is None and low is not None and high_so_far >= low:
        return "LOCKED_BY_OBSERVED_HIGH"
    if low is not None and high_so_far >= low:
        return "LOCKED_BY_OBSERVED_HIGH"
    return "UNRESOLVED_BUCKET"
...
def _exceed_upper_risk(weather: dict[str, Any], high: float) -> float:
    forecast = weather.get("forecast_high_f")
    current = weather.get("current_temp_f")
    risk = rebound_risk(weather) * 0.45 + volatility_risk(weather) * 0.25
    if forecast is not None:
        risk += max(0, forecast - high) * 18
    if current is not None:
        risk += max(0, current - high) * 12
    return risk
...
def weather_confidence(weather: dict[str, Any]) -> int:
    ...
    if weather.get("forecast_high_f") is None:
        score -= 25
    if weather.get("market_day_state") != "future" and weather.get("high_so_far_f") is None:
        score -= 15
    disagreement = weather.get("model_disagreement_f")
    if disagreement is not None:
        score -= min(25, disagreement * 5)
    elif weather.get("open_meteo_high_f") is None:
        score -= 8
...
def rebound_risk(weather: dict[str, Any]) -> int:
    if weather.get("settlement_source_status") == "cli_available" and weather.get("cli_high_f") is not None:
        return 5
    ...
    high_so_far = weather.get("high_so_far_f")
    current = None if weather.get("market_day_state") == "future" else weather.get("current_temp_f")
    if threshold is not None and max([v for v in [forecast, high_so_far, current] if v is not None], default=-999) >= threshold - 1:
        score += 35
    if forecast is not None and current is not None and forecast - current >= 3:
        score += 20
    if weather.get("pressure_trend") == "falling":
        score += 8
    if weather.get("humidity_trend") == "falling":
        score += 8
...
def final_label(
    ...
    if rebound >= 75 or volatility >= 80:
        return "AVOID"
    ...
    if warnings and edge < 55:
        return "WAIT"
    if edge >= 60 and confidence >= 70 and rebound < 55 and liquidity < 65 and crowding < 70:
        return "BUY WATCH"
...
def _reasoning(
    ...
    reasons = [
        f"Estimated weather probability is {estimated_probability}% with edge score {edge}.",
    ]
    if weather.get("settlement_source_status") == "cli_available" and weather.get("cli_high_f") is not None:
        reasons.append(
            f"NWS CLI report high is {weather['cli_high_f']:.1f}F "
            f"({weather.get('official_climate_product', 'CLI')})."
        )
    else:
        reasons.append("NWS CLI report high is not available yet; using observation/forecast fallback.")
    threshold = weather.get("threshold_f")
    high_so_far = weather.get("high_so_far_f")
    forecast = weather.get("forecast_high_f")
    ...
    if threshold is not None and high_so_far is not None:
        reasons.append(f"High so far is {high_so_far:.1f}F vs threshold {threshold:.1f}F.")
    if threshold is not None and forecast is not None:
        reasons.append(f"Forecast high is {forecast:.1f}F vs threshold {threshold:.1f}F.")
...
def _source_phrase(weather: dict[str, Any]) -> str:
    if weather.get("settlement_source_status") == "cli_available" and weather.get("cli_high_f") is not None:
        return f"NWS CLI high {weather['cli_high_f']:.1f}F"
    if weather.get("high_so_far_f") is not None:
        return "NWS observation high-so-far plus forecast fallback"
