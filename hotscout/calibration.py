...
def _select_forecast_by_date(conn, city, min_lead_days=0):
    ...
    by_date = {}
    for row in rows:
        by_date.setdefault(row["date"], []).append((row["lead_days"], row["forecast_high_f"]))

    result = {}
    for date, entries in by_date.items():
        min_lead = min(lead for lead, _ in entries)
        values = [value for lead, value in entries if lead == min_lead]
