...
def collect_city_set(
    ...
    *,
    ...
    for city_config in city_configs:
        ...
        scored = enrich_decision_layer(score_market(city, weather, market))
...
def _default_city_configs(city_configs: list[dict]) -> list[dict]:
    ...
    if _include_coastal():
        return city_configs
    filtered = [config for config in city_configs if not config.get("coastal_risk")]
    removed = [config["city"] for config in city_configs if config.get("coastal_risk")]
    if removed:
        print(f"Skipping coastal-risk cities by default: {', '.join(removed)}")
    return filtered
...
def _include_coastal() -> bool:
    return os.environ.get("KALSHI_INCLUDE_COASTAL", "").strip().lower() in {"1", "true", "yes", "y"}


def _skip_tomorrow() -> bool:
    return os.environ.get("KALSHI_SKIP_TOMORROW", "").strip().lower() in {"1", "true", "yes", "y"}
...
def main() -> None:
    ...
    print(f"Starting kalshi-weather-scout run {run_id}")
    ...
    if _skip_tomorrow():
        print("Skipping tomorrow city set because KALSHI_SKIP_TOMORROW is enabled.")
        tomorrow_rows, tomorrow_signals = [], []
    else:
        tomorrow_date = next_market_date()
        tomorrow_rows, tomorrow_signals = collect_city_set(
            _default_city_configs(city_configs_for_date(tomorrow_date)),
            conn,
            run_id,
            save_snapshots=False,
            label=f"tomorrow {tomorrow_date}",
        )
    ...
    write_dashboard_report(DASHBOARD_REPORT_PATH, scored_rows, all_signals, tomorrow_rows, tomorrow_signals)
    conn.close()
