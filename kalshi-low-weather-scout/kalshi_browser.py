...
def fetch_kalshi_market(city_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not market.get("contracts"):
        market["warnings"].append("No visible low-temperature contracts parsed from Kalshi page.")
    market["url"] = url
    return market
...
def _parse_visible_market_text(text: str, city_config: dict[str, Any]) -> dict[str, Any]:
    ...
    heading = next((line for line in lines if line.lower().startswith("lowest temperature")), None)
