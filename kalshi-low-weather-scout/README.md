# kalshi-low-weather-scout

Cheap MVP research tool for Kalshi daily low-temperature markets.
...
- The scanner prefers the exact NWS Climatological Report Daily (`CLI...`)
  products cited in Kalshi rules. If a CLI low is available, it is treated as
...
- Manual station config lives in `config.py` and maps the active low-temperature
  city markets to their official NWS stations/climate reports. Coastal and
  high-reversal-risk cities are intentionally excluded from the scanner.
