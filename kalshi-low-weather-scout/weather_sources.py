...
def parse_cli_report_text(text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "settlement_source_status": "cli_available" if low is not None else "cli_unavailable",
    }


def _parse_cli_minimum(text: str) -> float | None:
    for pattern in [
        r"^\s*MINIMUM\s+(-?\d{1,3})\b",
        r"^\s*MIN TEMP(?:ERATURE)?\s+(-?\d{1,3})\b",
        r"^\s*MIN\s+(-?\d{1,3})\b",
    ]:
    ...
    for pattern in [
        r"^\s*MAXIMUM\s+-?\d{1,3}\b.*?\bMINIMUM\s+(-?\d{1,3})\b",
        r"^\s*MAX\s+TEMP(?:ERATURE)?\s+-?\d{1,3}\b.*?\bMIN\s+TEMP(?:ERATURE)?\s+(-?\d{1,3})\b",
        r"^\s*MAX\s+-?\d{1,3}\b.*?\bMIN\s+(-?\d{1,3})\b",
    ]:
        match = re.search(pattern, text, flags=re.I | re.M)
        if match:
            return float(match.group(1))
...
def fetch_nws_observation_history(city_config: dict[str, Any]) -> dict[str, Any]:
    ...
    raw_low_so_far = min(temps) if temps else None
    return {
        "raw_low_so_far_f": raw_low_so_far,
        "low_so_far_f": _round_official_temp(raw_low_so_far),
...
def parse_digital_dwml_forecast(xml_text: str, target_date: str | None) -> dict[str, Any]:
    ...
    if not hourly_temps:
        hourly_pairs = _dated_value_pairs(root, "temperature", "minimum", layouts, target_date)
        hourly_temps = [value for _, value in hourly_pairs]
    low_pair = min(hourly_pairs, key=lambda item: item[1]) if hourly_pairs else None

    return {
        "forecast_low_f": min(hourly_temps) if hourly_temps else None,
        "forecast_low_time": low_pair[0] if low_pair else None,
...
def fetch_open_meteo(city_config: dict[str, Any]) -> dict[str, Any]:
    ...
    temps = _open_meteo_market_day_values(
        hourly.get("time", []),
        hourly.get("temperature_2m", []),
        city_config.get("market_date"),
        city_config.get("timezone", "UTC"),
    )
    clouds = _open_meteo_market_day_values(
        hourly.get("time", []),
        hourly.get("cloud_cover", []),
        city_config.get("market_date"),
        city_config.get("timezone", "UTC"),
    )
    open_low = min(temps) if temps else None
    return {
        "open_meteo_low_f": open_low,
        "open_meteo_cloud_change": _change(clouds[:12]),
        "source_urls": [url],
    }


def _open_meteo_market_day_values(
    times: list[Any],
    values: list[Any],
    ...
    for time_text, value in zip(times, values):
        ...
        if number is None:
            continue
        if market_date and isinstance(time_text, str):
            try:
                parsed = datetime.fromisoformat(time_text)
                if parsed.tzinfo is None:
                    local_date = parsed.date().isoformat()
                else:
                    local_date = parsed.astimezone(local_tz).date().isoformat()
            except ValueError:
                continue
...
def _empty_weather(city_config: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "market_date": city_config.get("market_date"),
        "contract_side": city_config.get("contract_side", "range"),
        "coastal_risk": city_config.get("coastal_risk", False),
        "current_temp_f": None,
        "raw_low_so_far_f": None,
        "low_so_far_f": None,
...
def _fill_derived_weather(
    ...
    observed_candidates = [
        value
        for value in [weather.get("current_temp_f"), weather.get("low_so_far_f")]
        if value is not None and not market_is_future
    ]
    observed_low = min(observed_candidates) if observed_candidates else None
    if cli_low is not None and target_date and cli_report_date and cli_report_date != target_date:
    ...
    elif cli_low is not None and target_date and not cli_report_date:
        weather["settlement_source_status"] = "cli_unverified_date"
        weather["warnings"].append(
            "NWS CLI report date could not be parsed; treating CLI low as unverified for this market date."
        )
    elif cli_low is not None and observed_low is not None and observed_low < cli_low - 1:
        weather["settlement_source_status"] = "cli_inconsistent"
        weather["warnings"].append(
            f"NWS CLI low {cli_low:.1f}F is above observed low/current {observed_low:.1f}F; treating CLI as preliminary or stale."
        )
    elif cli_low is not None and _market_day_still_open(city_config, now):
        weather["settlement_source_status"] = "cli_preliminary"
        weather["warnings"].append(
            "NWS CLI report matches the market date but the local weather day is still open; treating CLI low as preliminary."
        )
    elif cli_low is not None:
        weather["low_so_far_f"] = weather["cli_low_f"]
        weather["forecast_low_f"] = weather["cli_low_f"]
        weather["settlement_source_status"] = "cli_available"
    if weather.get("low_so_far_f") is None and not market_is_future:
        weather["low_so_far_f"] = weather.get("current_temp_f")
    ...
    if weather.get("forecast_low_f") is None:
        weather["forecast_low_f"] = weather.get("open_meteo_low_f")
    ...
    if market_is_future:
        ...
        if weather.get("forecast_low_f") is not None:
            weather["forecast_source_status"] = "future_hourly_forecast"
    elif weather.get("low_so_far_f") is not None or weather.get("current_temp_f") is not None:
    ...
    if weather.get("model_disagreement_f") is None:
        a = weather.get("forecast_low_f")
        b = weather.get("open_meteo_low_f")
    ...
    _fill_low_window(weather, city_config, now)
...
def _market_day_still_open(city_config: dict[str, Any], now: datetime | None) -> bool:
    ...
    return local_now.date() <= market_day
...
def _fill_low_window(
    ...
    if weather.get("settlement_source_status") == "cli_available" and weather.get("cli_low_f") is not None:
        weather["active_low_window"] = False
        return
    ...
    if market_date:
        try:
            ...
            if market_day > local_now.date():
                weather["active_low_window"] = True
                return
            active = local_now.date() <= market_day
        except ValueError:
            active = True
    ...
    if latest_time is None:
        active = True
        weather["warnings"].append(
            "Latest station observation timestamp is unavailable; keeping low-temperature window risk active."
        )
    else:
        ...
        if age_minutes > 90:
            active = True
            weather["warnings"].append(
                f"Latest station observation is stale ({age_minutes:.0f} minutes old); keeping low-temperature window risk active."
            )

    weather["active_low_window"] = active
...
def _trend(values: list[float]) -> str:
    if len(values) < 4:
        return "unknown"
    # NWS station observation feeds are newest-first; preserve that ordering when
    # comparing recent values against older values.
